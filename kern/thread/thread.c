...
thread_bootstrap(void) {
    ...
    p_list_lock = lock_create("processLock");
    
    curthread->pid = 1;
    
    process_list[1] = (struct p_list *)kmalloc(sizeof(struct p_list));
    if (process_list[1]==NULL){
        return ENOSPC;
    }
    process_list[1]->wait= cv_create("wait");
    process_list[1]->new_as = curthread->t_vmspace;
    process_list[1]->p_pid = -1;
    process_list[1]->exitstatus[0]=0;
...
thread_wakeup_one(const void *addr) {
    ...
    struct thread *chosen =array_getguy(sleepers, index);
    array_remove(sleepers, index);
    
    result=make_runnable(chosen);
    assert(result==0);  
     */
