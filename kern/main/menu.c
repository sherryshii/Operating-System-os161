...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &child);
        lock_acquire(p_list_lock);
        process_list[280]= (struct p_list *)kmalloc(sizeof(struct p_list));
        child->pid = 280;
        process_list[280]->new_as=child->t_vmspace;
        process_list[280]->wait = cv_create("wait");
        process_list[280]->exitstatus[0]=0;
        process_list[280]->p_pid=1;
        lock_release(p_list_lock);
	if (result) {
                kfree(process_list[280]);
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}
        sys_waitpid(child->pid,&a,0,&b);
